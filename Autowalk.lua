return function(WindUI, AutoWalkTab)
    ...
    -- Slider Speed (Tanpa pcall agar pasti muncul & Format benar)
    AutoWalkTab:Slider({
        ...
        Callback = function()
            ...
            playConn = RunService.Stepped:Connect(function()
                ...
                if isAutoWalkingToStart then
                    -- FASE 1: AUTO WALK MENUJU TITIK MULAI (Anti-Teleport)
                    hrp.Anchored = false
                    hum.AutoRotate = true
                    ...
                    if dist > 3 then
                        ...
                        WindUI:Notify({Title="Menempel", Content="Memulai sinkronisasi kecepatan rute!", Duration=1.5})
                        if StatusPara then pcall(function() StatusPara:SetDesc(string.format("Status: Berjalan (%dx Speed)", playSpeed)) end) end
                    end
                else
                    -- FASE 2: PLAYBACK RUTE (Smart CFrame & Velocity)
                    hrp.Anchored = false -- HARUS FALSE agar collision (Checkpoints) bekerja 100%!
